...
set OPENAI_MODEL=stepfun-ai/step-3.5-flash
...
cd /d C:\888\gemini\app\AudioSurf
chcp 932 > nul

call npx cc-haha "C:\888\gemini\app\AudioSurf ディレクトリに移動して作業してください。日本語で考えて行動してください。20task2.txtの内容を自動で実行して下さい。 実行エラーが報告された場合は、最適な方法で自動で修正し、成功するまで対応を続けてください。" --dangerously-skip-permissions --tui --auto
...
call npx openclaude "C:\888\gemini\app\AudioSurf ディレクトリに移動して作業してください。日本語で考えて行動してください。20task2.txtの内容を自動で実行して下さい。 実行エラーが報告された場合は、最適な方法で自動で修正し、成功するまで対応を続けてください。" --dangerously-skip-permissions --print
